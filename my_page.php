...
if($_GET["content"]){
    ...
    $parent_search = "and (parent_id = 907)";
$add_search = " $parent_search   $query_add";

$q1 = mysqli_query2("select * from page where  id = '907'");
...
?>
    <div class="container text-<?=$align?> <?=$dir?> cat-pgee blk-home" style="margin-bottom: 60px !important;">
        <div class="col-md-12 text-<?=$align?> <?=$dir?> w3-padding-32-h">
            <?
            $q= mysqli_query2("select src,description,e_date,publish_date,src_1,description_en,text,text_en,src_2,id,ext_link,type_page,name_en,name,cleanurl,cleanurl_en from page where m_main = 1 and active=1 ".$add_search."  order by the_order, id desc limit $start,$pagesize");
            if(mysqli_affected_rows2() > 0) {
                ?>
                <h2 class="text-center ge-flow-bold size_34 text-<?=$align?> title <?=$dir?>" style="margin-top: 32px">
                    <?=$r1["name".$ext]?>
                </h2>
                <h6 class="text-center ge-flow-bold w3-text-red size_16 text-<?=$align?> title <?=$dir?>">
                    <?=$label_laws_decision_members?>
                </h6>
                <?
                ...
                <div class="w3-margin-top w3-padding-top w3-center" style="margin-top: 30px">
                    <?php
                    nav_5("page", $pagesize, "mypage", $_GET["page"], " w3-margin-bottom btn", "btn background_brown  w3-margin-bottom", "where m_main = 1 and active=1 ".$add_search."  order by the_order, id desc");
                    ?>
                </div>
